...
-- actual shopify revenue by channel
shopify_by_channel as (
    select
        ...
        round(sum(case when financial_status = 'paid'
            and is_subscription = true then order_total else 0 end), 2)                     as subscription_revenue
    from orders
    group by channel
),
...
final as (
    select
        ...
        round(safe_divide(m.meta_reported_revenue - s.actual_revenue,
            s.actual_revenue) * 100, 1)                                                  as meta_overreport_pct,
        round(safe_divide(s.actual_revenue, m.total_spend), 2)                          as true_roas,
        round(safe_divide(m.meta_reported_revenue, m.total_spend), 2)                   as meta_reported_roas
